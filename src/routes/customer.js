...
// server/src/routes/customer.js - Updated to handle multiple projects
...
// Get customer dashboard data (all projects)
router.get('/dashboard', async (req, res) => {
  try {
    const { userId, phone } = req.user;
    ...
    // Search for ALL leads in Odoo CRM for this phone number
    let odooLeads = [];
    try {
      odooLeads = await odooService.fetchLeadByPhone(phone);
      console.log(`Found ${odooLeads.length} leads for phone: ${phone}`);
    } catch (odooError) {
      console.error('Odoo search error:', odooError);
      // Continue without Odoo data if service is unavailable
    }
    ...
    const dashboardData = {
      ...
      odooLead: odooLeads, // Return all leads as array
      projectStatus: odooLeads.length > 0 ? 'active' : 'not_found',
...
// Get specific project data by project ID
router.get('/dashboard/project/:projectId', async (req, res) => {
  try {
    ...
    // Search for ALL leads for this phone to verify access
    let userLeads = [];
    try {
      userLeads = await odooService.fetchLeadByPhone(phone);
    } catch (odooError) {
      console.error('Odoo search error:', odooError);
      return res.status(500).json({ message: 'Failed to fetch project data' });
    }

    // Check if the requested projectId belongs to this user
    const requestedProject = userLeads.find(
      lead => lead.id.toString() === projectId.toString()
    );
