...
router.post('/verify-otp', async (req, res) => {
  try {
    ...
    // ✅ NEW: Fetch ALL leadIds from Odoo using phone number
    let leadData = [];
    let leadIds = [];
    
    try {
      leadData = await odooService.fetchLeadByPhone(phone); // This returns an array
      if (leadData && leadData.length > 0) {
        leadIds = leadData.map(lead => lead.id);
        console.log(`✅ Found ${leadData.length} Odoo leads for phone ${phone}:`, leadIds);
      } else {
        console.log(`⚠️ No Odoo leads found for phone ${phone}`);
      }
    } catch (error) {
      console.error('❌ Error fetching leads from Odoo:', error);
      // Continue login even if Odoo fails - leadIds will be empty array
    }

    // ✅ ENHANCED: Update user with leadIds and verification status
    const updateData = {
      isVerified: true,
      lastLogin: new Date()
    };

    // Store leadIds permanently if found
    if (leadIds.length > 0) {
      updateData.leadIds = leadIds; // Store array of lead IDs
      updateData.leadInfo = leadData.map(lead => ({
        id: lead.id,
        name: lead.name,
        phone: lead.phone,
      ...
      updateData.projectCount = leadData.length;
    } else {
      updateData.leadIds = [];
      updateData.leadInfo = [];
      updateData.projectCount = 0;
    }

    await userDoc.ref.update(updateData);

    // ✅ ENHANCED: Generate JWT token with leadIds (if available)
    const tokenPayload = {
      userId: userDoc.id,
      phone: userData.phone,
      role: userData.role
    ...
    // Only add leadIds if found
    if (leadIds.length > 0) {
      tokenPayload.leadIds = leadIds; // Store array of lead IDs
      tokenPayload.projectCount = leadIds.length;
    }
    ...
    // ✅ ENHANCED: Response includes leads info and feedback status
    const response = {
      message: 'Login successful',
      token,
      user: {
    ...
    // Include leads info if found
    if (leadData.length > 0) {
      response.leads = leadData.map(lead => ({
        id: lead.id,
        name: lead.name,
        stage_id: lead.stage_id ? lead.stage_id[0] : null
      }));
      response.projectCount = leadData.length;
    }
...
router.get('/project/:projectId', authMiddleware, authController.getProjectById);
