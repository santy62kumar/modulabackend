...
class TrackingService {
  ...
  async processLead(leadData) {
    try {
      ...
      const existingProject = await firestoreService.findProject(normalizedPhone, odooLeadId);
  ...
  async createNewProject(projectData) {
    try {
      ...
      const projectId = await firestoreService.createProject(newProjectData);
  ...
  async checkFeedbackTriggers(projectData, newStageId) {
    try {
      ...
      if (newStageId === 22) {
        await firestoreService.updateFeedbackStatus(projectData.id, {
  ...
  async getProjectForCustomer(phone) {
    try {
      return await firestoreService.getProjectByPhone(phone);
    } catch (error) {
  ...
  async getProcessingStats() {
    try {
      const projects = await firestoreService.getAllActiveProjects();
